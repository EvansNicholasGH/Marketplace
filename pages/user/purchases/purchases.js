...
class ItemListings extends Component {
  ...
  static async getInitialProps(props) {   
     ...
     const list = await coinListing.methods.getUserPurchasesFA(addr).call();
     const itemCount = list.length;
     const properties = { allCars: [] };   
     if (!itemCount) return properties;    
     var onResolve = () => { };
     var itemsLoaded = 0;
  ...
  renderCar = (car, index) => {
    return <Card key={car[7]}><Item source={placeholder} type={car[1]} meta={car[0]} description={car[5]} price={car[4]} id={car[7]} key={car[7]} /></Card>;
  };
